$(document).ready(function(){
    ...
    $('#submit').bind('click',function(){
       ...
       if(verifytitle()&&verifycontent(ubbeditor)&&verifyqq()&&verifymanager()&&verifyphone()) {
        return true;
       }else{
        return false;
       }
...
function verifyqq(){
    ...
    var str = /^(\d+.\d+){1}$|^\d+$/;
    if(iflimit(qq,1,15)){
            $("#qq_c").text("内容不可为空").css("color","red");
            return false;
          }else if(!ifregex(qq,str)){
            $("#qq_c").text("只能输入数字").css("color","red");
            return false;
          }else{
            $("#qq_c").text("请输入数字").css("color","");
            return true;
          } 
...
function verifyrequire(){
    ...
    var str = /^\d*$|^[a-z]*$|^[A-Z]*$|^\s*$/;
    if(Checksensitive(require)){
            $('#require_c').text("请勿输入反动、迷信及不文明言论").css("color","red");
            return false;
        }else if(require.length == 0){
            $("#require_c").text("需求不可为空").css("color","red");
            return false;
        }else if(require.length > 10){
            $("#require_c").text("需求不可过长").css("color","red");
            return false;
        }else if(ifregex(require,str)){
            $("#require_c").text("请勿输入无意义的文字").css("color","red");
            return false;
        }else{
            $("#require_c").text("请输入需求").css("color","");
            return true;
        } 
}